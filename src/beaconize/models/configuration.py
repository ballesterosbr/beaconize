...
#   filename:  https://raw.githubusercontent.com/ga4gh-beacon/beacon-v2/refs/heads/main/framework/json/responses/beaconConfigurationResponse.json
#   timestamp: 2026-04-21T11:43:28+00:00
...
class MaturityAttributes(BaseModel):
    productionStatus: Optional[ProductionStatus] = Field(
        None,
...
class BasicElement(BaseModel):
    ...
    id: str = Field(..., description='A (unique) identifier of the element.')
    name: str = Field(..., description='A distinctive name for the element.')


class ElementWithDescription(BasicElement):
    description: Optional[str] = Field(
        None, description='A textual description for the element.'
    )
...
class ReferenceToAnSchema(ElementWithDescription):
    model_config = ConfigDict(
        extra='allow',
    )
...
class EntryTypeDefinition(BaseModel):
    ...
    aCollectionOf: Optional[List[BasicElement]] = Field(
        None,
    ...
    additionallySupportedSchemas: Optional[List[ReferenceToAnSchema]] = Field(
        None,
        description='List of additional schemas that could be used for this concept in this instance of Beacon.',
    )
    defaultSchema: ReferenceToAnSchema = Field(
    ...
    id: str = Field(
        ...
        examples=['biosample', 'individual', 'dataset'],
    )
    name: str = Field(..., description='A distinctive name for the element.')
    nonFilteredQueriesAllowed: Optional[NonFilteredQueriesAllowed] = Field(
        default_factory=lambda: NonFilteredQueriesAllowed.model_validate(True)
    )
    ontologyTermForThisType: Optional[OntologyTerm] = Field(
        None,
        examples=[
            {'id': 'EFO:0000542', 'label': 'individual'},
            {'id': 'OBI:0000747', 'label': 'material sample'},
        ],
    )
    partOfSpecification: str = Field(
        ...,
        description='This is label to group together entry types that are part of the same specification.',
        examples=['Beacon v2.0'],
    )


class EntryTypes(RootModel[Dict[str, EntryTypeDefinition]]):
    root: Dict[str, EntryTypeDefinition]


class BeaconConfigurationSchema(BaseModel):
    model_config = ConfigDict(
        extra='allow',
    )
    field_schema: FieldSchema = Field(..., alias='$schema')
    entryTypes: EntryTypes
    maturityAttributes: MaturityAttributes = Field(
        ..., description='Declares the level of maturity of the Beacon instance.'
    )
    securityAttributes: Optional[SecurityAttributes] = Field(
        None,
...
class Model(BaseModel):
    ...
    meta: BeaconInformationalResponseMeta = Field(
        ...,
    ...
    response: BeaconConfigurationSchema = Field(
